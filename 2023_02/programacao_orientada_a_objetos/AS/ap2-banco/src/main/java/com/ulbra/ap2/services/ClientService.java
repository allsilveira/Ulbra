...
import com.ulbra.ap2.services.dto.ClientResponseDTO;
...
public class ClientService {
    ...
    public ClientResponseDTO listById(UUID id) {
        try {
            Optional<Client> client = this.repository.findById(id);
            if (client.isEmpty()) {
                throw new NoRegisteredClients("Nenhum cliente encontrado");
            }
            return new ClientResponseDTO(client.get().getNome(), client.get().getIdade(), client.get().getProfissao());
        } catch(Exception error) {
            error.printStackTrace();
            throw new RuntimeException(error.getMessage());
        }
